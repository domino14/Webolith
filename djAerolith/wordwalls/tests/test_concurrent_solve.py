...
class WordwallsConcurrentSolveTest(TransactionTestCase):
    ...
    def test_guess_same_word(self):
        ...
        self.assertTrue(st["solvers"][word] in ["cesar", "user_4738", "user_131"])
    ...
    def test_guess_same_alphagram(self):
        ...
        threads = []
        word = "ELATIONS"
        threads.append(threading.Thread(target=guess_fn, args=("ELATIONS", user)))
        threads.append(threading.Thread(target=guess_fn, args=("INSOLATE", user_1)))
        threads.append(threading.Thread(target=guess_fn, args=("TOENAILS", user_2)))
        for t in threads:
            t.start()
        for t in threads:
            t.join()
