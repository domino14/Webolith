...
class WordwallsSavedListModesTest(WordwallsBasicLogicTestBase):
    ...
    def test_restart_unfinished_list(self):
        ...
        table_id = self.wwg.initialize_by_saved_list(
            ...
            self.user,
            word_list,
            SavedListForm.RESTART_LIST_CHOICE,
            240,
        )
        word_list = WordList.objects.get(name=LIST_NAME)
        self.assert_wl(
    ...
    def test_continue_unfinished_list(self):
        ...
        table_id = self.wwg.initialize_by_saved_list(
            ...
            self.user,
            word_list,
            SavedListForm.CONTINUE_LIST_CHOICE,
            240,
        )
        word_list = WordList.objects.get(name=LIST_NAME)
        self.assert_wl(
...
def blank_bingo_generator(
    length, lexicon_name, num_2_blanks, num_questions, max_answers
):
    if length == 7:
        return [
            {
                "q": "ABIPST?",
                "a": ["BAPTISM", "BAPTIST", "BITMAPS", "BAPTISE"],
            },
            {"q": "AIINTX?", "a": ["TAXIING"]},
            {"q": "CILMSU?", "a": ["CULTISM", "MUSICAL"]},
            {
                "q": "EIOORT?",
                "a": ["FOOTIER", "HOOTIER", "SOOTIER", "ZOOTIER", "ROOTIER"],
            },
            {"q": "DHIIOR?", "a": ["RHIZOID"]},
            {"q": "DIRSTW?", "a": ["WRISTED"]},
            {"q": "EEFGSU?", "a": ["REFUGES"]},
            ...
            {
                "q": "AEILSU?",
                "a": ["AUDILES", "INSULAE", "INULASE", "DUALISE"],
            },
            {
                "q": "ABEENR?",
                "a": ["ENABLER", "REBEGAN", "VERBENA", "BEANERY"],
            },
            {"q": "AMNOOS?", "a": ["MAROONS", "ONOMAST", "ROMANOS"]},
            {"q": "EEKLRT?", "a": ["KELTERS", "KESTREL", "SKELTER"]},
            {"q": "BNOORS?", "a": ["BRONCOS"]},
            {"q": "AEGIIM?", "a": ["IMAGINE"]},
            {"q": "EHILPS?", "a": ["HIPLESS", "HIRPLES", "PLENISH"]},
            {"q": "EHKNRU?", "a": ["HUNKIER", "HUNKERS"]},
            {"q": "AEIMOO?", "a": ["IPOMOEA"]},
            {
                "q": "EFNTT??",
                "a": ["FETTING", "FATTENS", "FITMENT", "FLATTEN"],
            },
            {
                "q": "CEIUV??",
                "a": ["CURSIVE", "INCURVE", "UVEITIC", "UNVOICE", "CURVIER"],
            },
        ]
    elif length == 8:
        return [
            ...
            {
                "q": "AAMRSTU?",
                "a": ["TIMARAUS", "AMATEURS", "TAMARAUS", "TAMBURAS"],
            },
            {"q": "ABINRSV?", "a": ["VIBRANTS"]},
            {
                "q": "IMOOPRS?",
                "a": ["IMPOROUS", "IMPOSTOR", "ISOMORPH", "PROMISOR"],
            },
            {"q": "EGINOSY?", "a": ["SEIGNORY", "HOSEYING", "MOSEYING"]},
            {
                "q": "ABDORTU?",
                "a": ["ABDUCTOR", "OBDURATE", "OUTBOARD", "TABOURED"],
            },
            {"q": "CEIIIOS?", "a": ["IDIOCIES"]},
            {
                "q": "EEPRSTX?",
                "a": ["EXCERPTS", "PREEXIST", "PRETEXTS", "SEXPERTS"],
            },
            ...
            {"q": "AEEMORT?", "a": ["MODERATE", "OVERTAME"]},
            {"q": "DEOOSTV?", "a": ["DOVECOTS"]},
            ...
            {
                "q": "DEOOOW??",
                "a": ["WOODLORE", "WOODNOTE", "WOODTONE", "ROSEWOOD"],
            },
            {
                "q": "ABEGIY??",
                "a": ["BELAYING", "EMBAYING", "GIGABYTE", "LESBIGAY"],
            },
        ]
...
class WordwallsChallengeBehaviorTest(WordwallsBasicLogicTestBase):
    ...
    def test_bingo_marathon_challenge(self):
        ...
        self.assertEqual(len(set([q["q"] for q in questions])), 100)
    ...
    @mock.patch(
        "wordwalls.challenges.gen_blank_challenges",
    ...
    def test_play_old_challenge(self):
        ...
        table_id = self.wwg.initialize_daily_challenge(
            self.user,
            Lexicon.objects.get(lexiconName="CSW19"),
            challenge,
            date(2016, 12, 8),
        )
