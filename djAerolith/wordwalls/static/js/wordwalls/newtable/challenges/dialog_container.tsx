...
const ChallengeDialogContainer = forwardRef<
...
>(
  (
    ...
    const [challengeLeaderboardData, setChallengeLeaderboardData] =
      useState<object>({});
    const [specialChallengeInfo, setSpecialChallengeInfo] = useState<
      ChallengeInfo[]
    >([]);

    // Use memoized date string to prevent unnecessary re-renders
    const currentDateString = useMemo(
      () => currentDate.format(DATE_FORMAT_STRING),
      [currentDate]
    );
    ...
    const handleChallengeSelected = (challID: number) => () =>
      onChallengeSelected(challID);

    // Pacific Time hooks for auto-updating
    const { maxDate } = useChallengeAvailability();

    // Refresh data when Pacific Time day changes
    const refreshData = useCallback(() => {
      loadChallengePlayedInfo();
      loadSpecialChallenges();
    }, [loadChallengePlayedInfo, loadSpecialChallenges]);

    usePacificDayChange(refreshData);
