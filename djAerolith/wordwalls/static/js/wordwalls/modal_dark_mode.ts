 ...
 */
function applyDarkModeToModal(modalElement: HTMLElement): void {
  ...
  const modalHeader = modalElement.querySelector('.modal-header') as HTMLElement;
  if (modalHeader) {
  ...
  if (modalBody) {
    ...
    headings.forEach((heading) => {
      ...
      headingElement.style.textShadow = '1px 1px 2px rgba(0, 0, 0, 0.5)';
    });
  }

  // Apply dark mode to modal footer
  const modalFooter = modalElement.querySelector('.modal-footer') as HTMLElement;
  if (modalFooter) {
 ...
 */
function removeDarkModeFromModal(modalElement: HTMLElement): void {
  ...
  if (modalBody) {
    ...
    headings.forEach((heading) => {
      ...
      headingElement.style.textShadow = '';
    });
  }
 ...
 */
function setupDarkModeModalObserver(): MutationObserver | undefined {
  if (!document.body.classList.contains('dark-mode')) return undefined;
          ...
          if (node.nodeType === 1) {
            ...
            if (
              ...
              && (element.classList.contains('modal')
                || element.classList.contains('solutions-modal'))
            ) {
              applyDarkModeToModal(element);
            } else {
              // Check for modals inside the added node
              const modals = element.querySelectorAll('.modal, .solutions-modal');
              modals.forEach((modal) => applyDarkModeToModal(modal as HTMLElement));
            }
          }
        }
      }
...
function applyDarkModeToExistingModals(): void {
 ...
 */
function removeDarkModeFromExistingModals(): void {
  const modals = document.querySelectorAll('.modal, .solutions-modal');
  modals.forEach((modal) => removeDarkModeFromModal(modal as HTMLElement));
}
