...
from base.utils import (
    generate_question_map_from_alphagrams,
...
def saved_lists_get(request):
    ...
    return response(
        {
            "lists": [sl.to_python_reduced(last_saved_human) for sl in lists],
            "count": lists.count(),
            "limits": {
                "total": limit,
    ...
    if request.method == "DELETE":
        ...
        if action == "continue":
            ...
            if l_obj["goneThruOnce"] is False:
                return response(
                    "Cannot quiz on first missed unless you have "
                    "gone through the entire quiz.",
                    status=400,
                )
...
def edit_saved_list(request, sl):
    ...
    if sl.numAlphagrams != body.get("numAlphagrams"):
        return response("The alphagrams for this list do not match.", status=400)
...
def question_map(request):
    ...
    try:
        sl = WordList.objects.get(user=request.user, id=request.GET.get("listId"))
    except WordList.DoesNotExist:
        return response("This list does not exist!", status=404)

    t1 = time.time()
    q_map = generate_question_map_from_alphagrams(
        sl.lexicon, json.loads(sl.origQuestions)
    )
...
def alphagram_history_search(request):
    ...
    lists = (
        WordList.objects.filter(user=request.user, lexicon=lex_obj)
        .filter(category=WordList.CATEGORY_ANAGRAM)
...
def get_q_summary(wl, q_idx, alphagram):
    ...
    if gone_thru_once:
        ...
        if not was_first_missed:
            ...
            q_summary.append(
                f"{alphagram} was correctly answered the first "
                "time through this list."
            )
        else:
            q_summary.append(
                f"{alphagram} was missed the first time through " "this list."
            )
    else:
        ...
        if not was_missed:
            ...
            if question_index > q_idx:
                q_summary.append(
                    f"List has not been completed, but "
                    f"{alphagram} was correctly answered."
                )
            else:
                q_summary.append(
                    f"List has not been completed, {alphagram} "
                    "has not been yet asked."
                )
        else:
            q_summary.append(
                f"List has not been completed, but {alphagram} " "was missed already."
            )
